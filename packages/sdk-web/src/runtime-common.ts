import type { WakerBundledRegistrationPolicy, WakerHeadArtifact, WakerRuntimeBackboneConfig } from "./types";

export const DEFAULT_CAPTURE_SAMPLE_RATE = 24_000;
const DEFAULT_BUNDLED_RUNTIME_BASE_PATH = "/waker-sdk-web/";
...
export type WakerWasmModuleShape = {
  ...
  trainTemporalConvHead(flattenedSequences: Float32Array, labels: Uint8Array, configJson: string): string;
};
...
export async function loadWakerWasmModule(moduleUrl: string, wasmBinaryUrl: string): Promise<WakerWasmModuleShape> {
...
export async function loadBundledBackboneAssets(
  ...
  const packageManifestPath = runtimeBackbone?.packageManifestPath ?? "backbone/model_manifest.json";
...
export async function loadHeadArtifactFromUrl(artifactUrl: string, baseUrl?: string): Promise<WakerHeadArtifact> {
