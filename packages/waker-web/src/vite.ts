...
type WakerWebRuntimeAssetsPluginOptions = {
  mountBase?: string;
};

const DEFAULT_MOUNT_BASE = "/waker-web/";
...
function resolveRequestFilePath(runtimeRoot: string, mountBase: string, requestUrl: string | undefined): string | null {
  ...
  if (!parsedPath.startsWith(mountBase)) {
    return null;
  }
...
export function wakerWebRuntimeAssetsPlugin(options: WakerWebRuntimeAssetsPluginOptions = {}) {
