    ...
    pub fn observe(
        ...
        if score >= threshold {
            ...
            if self.confirmation_hits >= required_hits.max(1) {
                self.confirmation_hits = 0;
                self.cooldown_until_ms = now_ms + (cooldown_seconds.max(0.0) as f64 * 1000.0);
                return true;
            }
            return false;
        }
...
mod tests {
    ...
    fn single_hit_triggers() {
        ...
        assert!(state.observe(0.8, 0.5, 1, 1.0, 1000.0));
    }

    #[test]
    fn below_threshold_does_not_trigger() {
        let mut state = DecisionState::new();
        assert!(!state.observe(0.3, 0.5, 1, 1.0, 1000.0));
    }
    ...
    fn cooldown_prevents_retrigger() {
        ...
        assert!(!state.observe(0.8, 0.5, 1, 1.0, 1500.0));
        // After cooldown
        assert!(state.observe(0.8, 0.5, 1, 1.0, 2100.0));
    }

    #[test]
    fn multi_hit_confirmation() {
        let mut state = DecisionState::new();
        assert!(!state.observe(0.8, 0.5, 3, 1.0, 1000.0));
        assert!(!state.observe(0.8, 0.5, 3, 1.0, 1100.0));
        assert!(state.observe(0.8, 0.5, 3, 1.0, 1200.0));
    }
    ...
    fn miss_resets_hit_count() {
        ...
        assert!(!state.observe(0.8, 0.5, 3, 1.0, 1100.0));
        // Miss resets
        assert!(!state.observe(0.3, 0.5, 3, 1.0, 1200.0));
        // Need 3 more hits now
        assert!(!state.observe(0.8, 0.5, 3, 1.0, 1300.0));
    }
}
